...
/// Convert a repo into a co-located repo.
#[derive(clap::Args, Clone, Debug)]
pub struct GitColocateArgs {
    /// Reverses the process and converts co-located repo into a regular `jj`
    /// repo.
    #[arg(long)]
    undo: bool,
...
pub fn cmd_git_colocate(
    ...
    if workspace_command.working_copy_shared_with_git() {
        writeln!(ui.status(), r#"This repo is already colocated; nothing to do"#);
        return Ok(());
    }

    workspace_command.check_working_copy_writable()
    let workspace_root = workspace_command.workspace_root();
    let gitignore = workspace_command.base_ignores()?;
